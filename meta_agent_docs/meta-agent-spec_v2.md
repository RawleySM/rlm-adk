...
## 5. The Neuro-Symbolic Spectrum, HRL, & DAG Integration
...
### 5.3 DAG Integration & Explicit Reasoning
The `rlm_adk` architecture natively supports and is enhanced by explicit Directed Acyclic Graph (DAG) execution engines. A DAG provides the perfect structural scaffolding (the Game Tree) for the meta-layer.
*   **Nodes as Information Sets:** Every node within a DAG represents a distinct Information Set. When the Orchestrator executes a DAG, it queries the VAD-CFR router at *each node* to select the optimal tool for that specific step.
*   **Solving Credit Assignment:** If a complex, multi-step workflow fails, the DAG structure isolates the failure point. VAD-CFR applies negative regret *only* to the tool used at the failing node, leaving the utility of successful upstream nodes intact.
*   **Dynamic DAG Generation:** The SHOR-PSRO Oracles can be tasked with an Information Set like `TaskType.PLAN_WORKFLOW` to dynamically generate novel DAG structures (e.g., as JSON/YAML) tailored to specific requests. The generated DAG itself becomes an "Action" evaluated by VAD-CFR, forcing the system to evolve better planning structures over time.
