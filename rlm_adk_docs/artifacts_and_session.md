...
- **2026-03-13 12:15** — `services.py`: New file. ADK CLI service registry overrides built-in `sqlite://` and `file://` schemes so `adk run rlm_adk` gets WAL-pragma'd sessions and file artifacts automatically. Added CLI service registration notes to sections 1 and 3.
