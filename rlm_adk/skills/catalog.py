...
class PromptSkillRegistration:
    ...
    repl_globals_factory: Callable[[], dict[str, Any]] | None = None
...
def _repomix_globals_factory() -> dict[str, Any]:
    """Lazy import of repomix helpers to avoid pulling heavy deps at catalog import time."""
    from rlm_adk.skills.repomix_helpers import pack_repo, probe_repo, shard_repo

    return {"probe_repo": probe_repo, "pack_repo": pack_repo, "shard_repo": shard_repo}
...
PROMPT_SKILL_REGISTRY: dict[str, PromptSkillRegistration] = {
    REPOMIX_SKILL.frontmatter.name: PromptSkillRegistration(
        ...
        repl_globals_factory=_repomix_globals_factory,
    ),
...
def collect_repl_globals(
    enabled_skills: Iterable[str] | None,
) -> dict[str, Any]:
    """Collect REPL globals from enabled skills' ``repl_globals_factory`` callables.

    Iterates enabled skills in registry order; calls each factory and merges
    the returned dicts.  Skills without a ``repl_globals_factory`` are skipped.
    """
    merged: dict[str, Any] = {}
    for name in normalize_enabled_skill_names(enabled_skills):
        reg = PROMPT_SKILL_REGISTRY[name]
        if reg.repl_globals_factory is not None:
            merged.update(reg.repl_globals_factory())
    return merged
